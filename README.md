...
> `CLAPBOT_FILES_ROOT` を未設定にした場合は production / dev とも `<cwd>/files` がデフォルトの公開対象になり、
> ディレクトリが存在しなければ `/api/files` は 404 を返します。意図しないディレクトリを露出しないよう、
> production では絶対パスで明示的に指定することを推奨します。
